...
- 资产管理：实体/虚拟资产、预设+自定义分类、敏感字段端到端加密（AES-256-GCM，每资产独立密钥 AK，主密钥/继承密钥包装；老资产自动回退主密钥解密）
- 继承（dead man's switch）：不登录升级提醒（免费 30/60/90/120 天，会员 7/14/30/60 天）→ 触发交接 → 继承人凭 event_key + 访问码双因子领取密钥；三重取消窗口（触发前登录 / 领取前 / 领取后登录反转）；**资产级继承**：资产可绑定多个继承人并各自设置触发天数，触发按资产产生事件，继承人领取只拿指定资产密钥
...
- 安全：APP 锁（PIN + 生物识别）、手动锁定（主页 AppBar 一键锁定）、免费/会员权益（免费资产上限 50）
