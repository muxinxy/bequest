...
func newMux(db *sql.DB) *http.ServeMux {
	...
	mux.Handle("GET /api/v1/categories/{id}/inheritors", requireAuth(handleListCategoryInheritors(db)))
	mux.Handle("POST /api/v1/categories/{id}/inheritors", requireAuth(handleCreateCategoryInheritor(db)))
	mux.Handle("DELETE /api/v1/categories/{id}/inheritors/{iid}", requireAuth(handleDeleteCategoryInheritor(db)))
	mux.Handle("GET /api/v1/inheritors/{id}/assets", requireAuth(handleListInheritorAssets(db)))
	...
	mux.Handle("GET /api/v1/settings/inheritance", requireAuth(handleGetInheritanceToggle(db)))
	mux.Handle("PUT /api/v1/settings/inheritance", requireAuth(handlePutInheritanceToggle(db)))
